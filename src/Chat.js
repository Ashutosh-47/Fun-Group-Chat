...
export default function Chat({ userName }) {
  ...
  useEffect(() => {                          
    ...
    if (groupId) {
      ...
      const getGroup = onSnapshot(doc(db, "groups", groupId), (doc) => {  setGroupName(doc.data().name)  })
      ...
      const getMessage = onSnapshot( q , (snapshot) => {
        ...
        snapshot.docs.forEach((doc) => {  msgList.push({ ...doc.data() }) });            
  ...
  const sendMessage = async (e) => {
    ...
    if (input == "") {    return alert("Please enter your message") }
